...
public class TencentcsHttpServiceAdapter implements HttpService {
    ...
    private static final String TENCENTCS_API_DEV_URL = "https://test-iotvideotencentcloudapi.cloudlinks.cn";
    ...
    public void init(String accessId, String secretKey, String token) {
        ...
        mTencentcsHttpServiceFactory = new TencentcsHttpServiceFactory(accessId, secretKey, token, hostName);
        LogUtils.i(TAG,"getUrl:" + getUrl() + "; hostName:" + hostName);
    ...
    private String getHostName() {
        String urlString;
        if (UrlHelper.getInstance().getServerType() == UrlHelper.SERVER_RELEASE) {
            urlString = TENCENTCS_API_URL;
        } else {
            urlString = TENCENTCS_API_DEV_URL;
        }
        int startIndex = urlString.indexOf("//") + 2;
        int endIndex = urlString.lastIndexOf(":");
        String hostName;
        if (startIndex < 0) {
            hostName = "iotvideo.tencentcloudapi.com";
        }else{
    ...
    public void deviceBind(String devId,
        ...
        deviceBind(tid, forceBind,null, subscriberListener);
    }

    @Override
    public void deviceBind(String devId, boolean forceBind, String bindToken,final SubscriberListener subscriberListener) {
        JsonObject jsonObject = new JsonObject();
        jsonObject.addProperty("AccessId", mAccessId);
        jsonObject.addProperty("Tid", devId);
        jsonObject.addProperty("Role", forceBind ? "owner" : "guest");
        jsonObject.addProperty("ForceBind", forceBind);
        jsonObject.addProperty("bindToken", bindToken);
        final Observable<JsonObject> observable = mHttpInterface.tencentcsApi(
                "CreateBinding", "2019-11-26", jsonObject);
        final Observer<JsonObject> subscriber = new Observer<>(new SubscriberListener() {
            @Override
