...
public class DeviceListFragment extends BaseFragment implements View.OnClickListener {
                ...
                switch (item.getItemId()) {
                    ...
                    case R.id.action_menu_unbind:
                        /*if (BuildConfig.DEBUG) {
                            Intent vasIntent = new Intent(getActivity(), SdkTestActivity.class);
                            vasIntent.putExtra("deviceID", device.getDevId());
                            startActivity(vasIntent);
                            break;
                        }*/
                        showTipDialog(getResources().getString(R.string.device_delete), getResources().getString(R.string.device_delete_tip), new CancelOrConfirmListener() {
                            @Override
                            public void onConfirm() {
                                LogUtils.d(TAG, "action_menu_unbind onConfirm");
                                dismissTipDialog();
                                if (device.getRelation() == 1) {
                                    unbindDevice(device);
                                } else {
                            ...
                            public void onCancel() {
                                LogUtils.d(TAG, "action_menu_unbind onCancel");
                                dismissTipDialog();
                            }
                        });
    ...
    private void queryDeviceList() {
        if (AccountSPUtils.getInstance().isAnonymousUser(getContext())) {
            ...
            final String multi_tid = AccountSPUtils.getInstance().getTid(getContext());
            // 如果是多个匿名设备同时登录，设备ID之间是拆分开的，中间使用","隔开
            if (!TextUtils.isEmpty(multi_tid) && multi_tid.contains(IoTVideoSdkConstant.AnonymousLogin.MULTI_DEV_SPLIT_REGEX)) {
                String[] tidList = multi_tid.split(IoTVideoSdkConstant.AnonymousLogin.MULTI_DEV_SPLIT_REGEX);
                ...
                LogUtils.i(TAG, "queryDeviceList: invalid device id");
            }
    ...
    private void devIdConvert() {
        if (null == mDeviceInfoList || mDeviceInfoList.isEmpty()) {
            return;
        }
        for (DeviceList.Device device : mDeviceInfoList) {
            if (null != device && !TextUtils.isEmpty(device.getDevId()) && device.getDevId().contains("$")) {
                device.setDevId(String.valueOf(IoTP2PUtils.thirtyTwoToDecimal(device.getDevId().substring(1))));
            }
        }
    }
    ...
    private void registerNotify() {
        IoTVideoSdk.getMessageMgr().addModelListener(data -> mAdapter.notifyDataSetChanged());
    }
