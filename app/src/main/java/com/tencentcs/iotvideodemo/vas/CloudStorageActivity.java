...
public class CloudStorageActivity extends BaseActivity implements View.OnClickListener {
    ...
    public void onClick(View view) {
        if (view.getId() == R.id.buy_start_time) {
            ...
            if (null != mDevice && 1 != mDevice.getRelation()) {
                Snackbar.make(mTvBuyStartTime, R.string.you_are_not_owner, Snackbar.LENGTH_LONG).show();
                return;
            }
            queryBuyedService();
        }
    ...
    private void queryBuyedService() {
        mVasService.getCloudStatus(mDevice.getDevId(), new SubscriberListener() {
            @Override
            public void onStart() {
            ...
            public void onSuccess(@NonNull JsonObject response) {
                ...
                CloudStorageInfo storageInfo = JSONUtils.JsonToEntity(response.toString(), CloudStorageInfo.class);
                if (storageInfo != null && storageInfo.data != null) {
...
//                    stringBuilder.append("套餐状态："+ storageInfo.getServiceStateInfo());
            ...
            @Override
            public void onFail(@NonNull Throwable e) {
                LogUtils.i(TAG, "onFail(:" + e.toString());
            }
        });
