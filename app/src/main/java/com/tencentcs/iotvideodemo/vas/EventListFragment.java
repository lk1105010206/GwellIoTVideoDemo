...
public class EventListFragment extends BaseFragment implements View.OnClickListener, SimpleRecyclerViewAdapter.OnItemClickListener {
    ...
    private void queryEventList(long startTime, long endTime) {
        ...
        mParentInstance.getmVasService().getEventListWithDeviceId(mParentInstance.getmDevice().getDevId(), startTime, endTime, null, 50, null, true, getCloudServiceId(), new SubscriberListener() {
            @Override
            public void onStart() {
        ...
        mParentInstance.getmVasService().getVideoDateListWithDeviceId(mParentInstance.getmDevice().getDevId(), 28800, getCloudServiceId(), new SubscriberListener() {
            @Override
            public void onStart() {
            ...
            @Override
            public void onSuccess(@NonNull JsonObject response) {
                LogUtils.i(TAG, "getCloudEventExistDateList:" + response);
            }

            @Override
            public void onFail(@NonNull Throwable e) {
                LogUtils.i(TAG, "getCloudEventExistDateList onFail:" + e.getMessage());
            }
        });
    }
    ...
    private void queryEventInfo(final String alarmId) {
        mParentInstance.getmVasService().queryEventInfo(mParentInstance.getmDevice().getDevId(), alarmId, new SubscriberListener() {
            ...
            public void onSuccess(@NonNull JsonObject response) {
                ...
                final EventInfo eventInfo = JSONUtils.JsonToEntity(response.toString(), EventInfo.class);
                if (eventInfo == null) {
                    Snackbar.make(mBtRefresh, "invalid data", Snackbar.LENGTH_LONG).show();
                    return;
                }
        ...
        mParentInstance.getmVasService().deleteEventsWithDeviceId(mParentInstance.getmDevice().getDevId(), alarmIdList, new SubscriberListener() {
            ...
            public void onSuccess(@NonNull JsonObject response) {
                ...
                if (response.has("code") && 0 == response.get("code").getAsInt()) {
                    Snackbar.make(mBtRefresh, "delete event successful， alarmId:" + alarmId, Snackbar.LENGTH_LONG).show();
    ...
    public void deleteEvent(long startTime, final String alarmId) {
        List<Long> timeList = new ArrayList<>();
        timeList.add(startTime);
        mParentInstance.getmVasService().deleteEventsWithStartTime(mParentInstance.getmDevice().getDevId(), timeList, new SubscriberListener() {
            @Override
            public void onStart() {
            ...
            public void onSuccess(@NonNull JsonObject response) {
                ...
                if (response.has("code") && 0 == response.get("code").getAsInt()) {
                    Snackbar.make(mBtRefresh, "delete event successful, startTime:" + startTime, Snackbar.LENGTH_LONG).show();
                    mEventList.remove(alarmId);
                    mAdapter.notifyDataSetChanged();
                } else {
                    Snackbar.make(mBtRefresh, "invalid data", Snackbar.LENGTH_LONG).show();
